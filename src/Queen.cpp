...
Chessman::Move_type Queen::cell_access(const int &x_first, const int &y_first, const int &x_second, const int &y_second, std::array<std::array<Cell, 8>, 8> &board)
{
    if (y_first == y_second)
    {
...
Positive_Score Queen::move_score(const int &x, const int &y, std::array<std::array<Cell, 8>, 8> &board)
{
...
std::string Queen::random_cell(const int& x, const int& y, std::array<std::array<Cell, 8>, 8> & board)
{
    move_access.clear();
    for (size_t i = 1, j = 1; x + i < 8 && y + j < 8; i++, j++)
    {
